...
Corner markers run up the verge and round the bend: small white blocks either side, 0.8 m long every 3.2 m, starting 16 m before the turn and carrying on through it to the exit. They belong to the circuit and to nothing else. A marker says where the corner is and nothing about how to drive it, so nothing about them knows what the car can do: the same curvature the road is lofted over says where the corners are, bends less than 6 m apart count as one so a chicane is one run rather than four, and each stretch of marked road is laid from its head, so the spacing is exact all the way down it and the last block of the approach lands on the corner entry itself. They are paint on the loft rather than props beside it — one more strip of the same cross-section, white where a marker falls and grass everywhere else — so there is nothing to place, nothing to light and nothing to clear away when the circuit changes.

Carrying them through the corner is the point of them. The dots ahead are the shape of the bend before you can see the shape of the bend — where it tightens they crowd up on the inside, where it opens they run away from you — and the part of a corner you cannot see is the part after the entry. The approach reaches 16 m back because the car's hardest stop, top speed down to the tightest corner the game allows, takes 15.1 m, so the first block of a run is the brakes for the slowest corners and the ones after it are the brakes for everything quicker. They are all the same block the same distance apart because a dotted line is read as a rhythm, and one block longer than the next, or further from it, has to be identified before it can be read. Spielberg carries 107 of them, Monza 179 and Spa 207.
