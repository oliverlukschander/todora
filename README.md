...
Braking markers count you into the corners: three white blocks on the verge either side, all the same, 4.8 m apart — two car lengths — with the last of them 4.8 m from the turn. They belong to the circuit and to nothing else. A marker says how far there is to go and nothing about how to drive it, so nothing about them knows what the car can do: the same curvature the road is lofted over says where the corners are, bends less than 6 m apart count as one so a chicane gets one set rather than four, and a marker is only laid where the road is clear all the way from it to the corner. A corner with another corner close behind it gets fewer, and one with a corner right behind it gets none, because you are already braking. They are paint on the loft rather than props beside it — one more strip of the same cross-section, white where a marker falls and grass everywhere else — so there is nothing to place, nothing to light and nothing to clear away when the circuit changes.

The set reaches 14.4 m out because that is about as far back as braking ever starts: top speed down to the tightest corner the game allows takes 15.1 m. So the first marker is the brakes for the slowest corners, the second for the medium ones, the third for the quick ones, and none of them for what the car holds flat. They are identical and evenly spaced on purpose — you count markers rather than read them, and a set where one is bigger than the next, or further from it, has to be identified before it can be counted. Spielberg carries four full sets, Monza four and Spa eight.
...
| `src/track/` | `circuits/` is one file per circuit, each a real trace in real metres, and `circuits/mod.rs` is the list the track key walks; `ribbon.rs` turns a trace into a centreline, opening the corners too tight to loft and exaggerating the ones too shallow to drive; `boards.rs` reads the corners back off it and marks the run into them; `profile.rs` is the cross-section — the mesh, the height under a wheel and the grip under it all read from one table — and fits it to what a circuit can carry; `mod.rs` is the `Track` the game asks where the ground is, and the switch that builds the next one. |
